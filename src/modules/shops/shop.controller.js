...
export const getShops = async (
  ...
  const query =
    shopListQuerySchema.parse(
      req.query,
    );

  const result =
    await listShopsService(query);
...
export const getShopBySlug = async (
  ...
  const { slug } =
    shopSlugSchema.parse(
      req.params,
    );

  const shop =
    await getShopBySlugService(
      slug,
    );
...
export const updateShopStatus =
  async (req, res) => {
    ...
    const data =
      updateShopStatusSchema.parse(
        req.body,
      );

    const shop =
      await updateShopStatusService({
        shopId,
        status: data.status,
      });
